...
\definecolor{sqlstring}{RGB}{0,128,0}
\definecolor{sqlcomment}{RGB}{128,128,128}
...
\lstdefinestyle{sqlstyle}{
    ...
    numbers=none,
    morekeywords={SERIAL, BIGSERIAL, UUID, BOOLEAN, TIMESTAMP, TIMESTAMPTZ, 
                  DECIMAL, VARCHAR, TEXT, DATE, BIGINT, INTEGER, INT,
                  VECTOR, ENUM, CHECK, REFERENCES, DEFAULT, UNIQUE,
                  gen_random_uuid, CURRENT_TIMESTAMP, PRIMARY, KEY,
                  CREATE, TABLE, INDEX, ON, USING, WITH, ivfflat,
                  vector_cosine_ops, lists, CASCADE, NOT, NULL, IN, AND}
}

\lstset{style=sqlstyle}

% Custom List of Diagrams
\newcommand{\listdiagramname}{List of Diagrams}
\newlistof{diagram}{dia}{\listdiagramname}
\newcommand{\diagram}[1]{%
    \refstepcounter{diagram}
    \addcontentsline{dia}{diagram}{\protect\numberline{\thediagram}#1}\par}
...
\begin{titlepage}
    ...
    Huzaifa Sarfaraz\\
    \vspace{0.5cm}
    \textbf{KG Committee Members:}\\
    Dr. Aimal Rextin, Dr. Hirra Anwar\\
    }
    \vfill
...
\caption{Document Revision History} \label{tab:revision-history} \\
\hline
...
\hline
\endhead
Finasaan Team & December 10, 2024 & Initial draft created from SRS document; defined system architecture and data models & 1.0 \\
\hline
M. Muzamil Kaleem & December 12, 2024 & Added activity diagrams, DFDs, and state transition diagrams; refined database schema & 1.1 \\
\hline
M. Hadi Khan & December 16, 2024 & Completed UI design chapter with screenshots; added screen objects and actions & 1.2 \\
\hline
S. Aman Hussain & December 16, 2024 & Finalized data dictionary; added vector embeddings storage section & 1.3 \\
\hline
\end{longtable}
...
\caption{Application Evaluation History} \label{tab:evaluation-history} \\
\hline
...
\vspace{1cm}
\noindent\textbf{Advised by}\\
\textbf{Dr. Farzana Jabeen}\\
...
The system leverages Next.js for frontend, Hono.js for backend, and PostgreSQL with pgvector extension for all structured data and vector embeddings. Cloudflare provides CDN, security infrastructure, R2 for object storage, and KV for distributed caching.
...
\begin{enumerate}
    ...
    \end{itemize}
    \item \textbf{Data Layer:} Unified database architecture:
    \begin{itemize}
        \item PostgreSQL with pgvector: All structured data (user accounts, portfolios, financial data, transactions, vector embeddings for RAG knowledge base)
        \item Cloudflare R2: Object storage for documents, PDFs, and media assets
        \item Cloudflare KV: Distributed edge caching for sessions, frequently accessed data, and real-time features
        \item Cloudflare Secret Store: Secure storage for API credentials and sensitive configuration
    \end{itemize}
...
\end{figure}
\diagram{Box-and-Line Architecture Diagram}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Data Layer:} Unified database architecture optimized for performance and scalability:
    \begin{itemize}
        \item PostgreSQL with pgvector: Primary database for all structured data and vector embeddings for semantic search and RAG
        \item Cloudflare R2: Object storage for documents, PDFs, and media assets
        \item Cloudflare KV: Distributed edge cache for sessions, frequently accessed data, and real-time features
        \item Cloudflare Secret Store: Encrypted storage for API credentials and sensitive configuration data
    \end{itemize}
...
This diagram walks through what happens when a new user signs up for Finasaan. The flow starts with the user entering their basic info like email and password. The web app validates everything first -- if the email format's wrong or password's too weak, it kicks them back with an error. Once validation passes, the backend checks if that email already exists in our database. New emails get a user account created, and we fire off a verification email. The user clicks the link, backend confirms verification, and we enable their account. Pretty standard stuff, but we've added a retry option in case the verification link expires or something goes wrong.
...
\end{figure}
\diagram{User Registration Activity Diagram}
\subsection{Financial Data Aggregation and Update Activity Diagram}

This one shows how we keep our financial data fresh. The job can kick off either manually (admin triggers it) or automatically through scheduled cron jobs. First thing we do is check if the external API is up -- if it is, we just hit the API directly. If not, we fall back to scraping. For lightweight scrapes, our admin worker handles it internally. Heavy jobs get offloaded to AWS Lambda so we don't block the main worker. All the scraped data goes through validation and normalization before we dump it into PostgreSQL and R2 storage. The interesting bit is the embedding generation -- after storing raw data, we generate vector embeddings through our AI pipeline (Groq via the AI Gateway) and store those in pgvector for semantic search. If anything fails along the way, we notify the admin and update the job metadata either way.
...
\end{figure}
\diagram{Financial Data Aggregation Activity Diagram}
\subsection{AI Chatbot Query Processing Activity Diagram}

This is probably the most complex flow in our system. When a user submits a question, the frontend first validates the input (can't have empty messages or super long queries). Valid requests go to our Hono backend where we authenticate the user and check rate limits. Then we fetch the user's profile -- checking KV cache first, falling back to PostgreSQL on cache miss. Here's where it gets interesting: we analyze the query complexity. Simple chitchat goes straight to prompt construction, but knowledge-heavy questions trigger our RAG pipeline. The RAG flow generates an embedding for the query, does a vector search in pgvector to find relevant chunks from our knowledge base, and attaches those as context. The constructed prompt (with user level info and retrieved context) gets sent to Groq. We stream the response back, adapt the tone based on user's knowledge level, append source citations, and render it with proper Markdown/LaTeX formatting. Error handling's built in throughout -- timeouts or API failures get caught and displayed gracefully.
...
\end{figure}
\diagram{AI Chatbot Query Processing Activity Diagram}
...
\end{figure}
\diagram{Context Diagram (Level 0 DFD)}
...
\end{figure}
\diagram{Level 1 DFD - System Decomposition}
...
\end{figure}
\diagram{Level 2 DFD - Selected Process Decomposition}
...
\end{figure}
\diagram{Data Ingestion State Transition Diagram}

\subsection{User Research Session State Machine}

\begin{figure}[htb]
    \centering
    \includegraphics[width=\textwidth]{images/user_research_state_machine.png}
    \caption{User Research Session State Machine.}
    \label{fig:user-research-session-state-machine}
\end{figure}
\diagram{User Research State Transition Diagram}
% Chapter 5: Data Design
\chapter{Data Design}

Finasaan's information domain is transformed into data structures using a unified PostgreSQL database with pgvector extension. The system stores all structured relational data (users, portfolios, financial records) and vector embeddings for the RAG-based AI system in PostgreSQL, complemented by Cloudflare R2 for object storage and Cloudflare KV for distributed caching.
...
\begin{figure}[htb]
    ...
    \caption{Entity-Relationship Diagram (ERD) of Finasaan's PostgreSQL Database.}
...
\section{Vector Embeddings Storage}

\subsection{Knowledge Base Vectors in PostgreSQL}
Vector embeddings are stored in PostgreSQL tables using the pgvector extension, enabling efficient similarity search for RAG operations. Embeddings are stored as 768-dimensional vectors alongside metadata for retrieval.
...
\subsection{Cache Layer and Object Storage Entities}

\textbf{Cloudflare KV Cache Keys:}
\begin{itemize}
...
\textbf{Cloudflare R2 Object Storage:}
\begin{itemize}
    \item documents/ - PDFs and educational materials
    \item reports/ - Generated user reports and exports
    \item media/ - Images, logos, and media assets
    \item backups/ - Database and data backups
\end{itemize}

\textbf{Cloudflare Secret Store:}
\begin{itemize}
    \item API credentials for external services (PSX, MUFAP, insurance providers)
    \item Database connection strings and credentials
...
The following screenshots illustrate the actual implementation of the Finasaan user interface, highlighting the user journey from authentication to the main learning dashboard.

\subsection{Authentication and Account Recovery}

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.40\textwidth]{images/ui/login.png}
    \hspace{0.05\textwidth}
    \includegraphics[width=0.40\textwidth]{images/ui/reset_password.png}
    \caption{Login Interface (left) and Password Recovery Screen (right)}
    \label{fig:auth_screens}
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.40\textwidth]{images/ui/verify_otp.png}
    \caption{OTP Verification Screen}
    \label{fig:otp_screen}
\end{figure}

\textit{Screen Description: A split-screen layout featuring high-quality imagery on the left with the slogan ``Learn. Invest. Grow'' and the authentication form on the right.}
\begin{itemize}
    \item Login Interface: Clean input fields for Email and Password, a prominent green ``Login'' button, and options for ``Continue with Google'' or ``Sign Up''.
    \item Password Recovery: A dedicated ``Reset Password'' screen allowing users to securely update credentials with ``New Password'' and ``Confirm Password'' fields.
    \item Visual Identity: Consistent branding with the Finasaan logo in the top-left and deep green accents reflecting the financial growth theme.
\end{itemize}

\subsection{Intelligent Onboarding Journey}

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.28\textwidth]{images/ui/onboarding_1.png}
    \hspace{0.02\textwidth}
    \includegraphics[width=0.28\textwidth]{images/ui/onboarding_2.png}
    \hspace{0.02\textwidth}
    \includegraphics[width=0.28\textwidth]{images/ui/onboarding_3.png}
    \caption{Onboarding Journey - Steps 1-3}
    \label{fig:onboarding_1_3}
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.28\textwidth]{images/ui/onboarding_4.png}
    \hspace{0.02\textwidth}
    \includegraphics[width=0.28\textwidth]{images/ui/onboarding_5.png}
    \hspace{0.02\textwidth}
    \includegraphics[width=0.28\textwidth]{images/ui/onboarding_6.png}
    \caption{Onboarding Journey - Steps 4-6}
    \label{fig:onboarding_4_6}
\end{figure}

\textit{Screen Description: A multi-step, dark-themed onboarding wizard designed to profile the user and tailor the learning experience.}
\begin{itemize}
    \item Personalization: The system asks ``What best describes you?'' with a dropdown menu (Student, Employed, Self Employed, Homemaker, etc.) to adapt content complexity.
    \item Interest Selection: A grid of selectable topic chips (e.g., ``Mutual Funds'', ``Stock Market \& PSX'', ``Taxation'', ``Retirement Planning'') allowing users to customize their feed.
    \item Navigation: Simple ``Back'' and ``Continue'' controls ensuring a smooth setup process without overwhelming the user.
\end{itemize}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.85\textwidth]{images/ui/dashboard_basic.png}
    \caption{Main Dashboard Interface with Sidebar Navigation}
    \label{fig:dashboard}
\end{figure}

\textit{Screen Description: The central hub of the application, featuring a sidebar navigation layout for easy access to all modules.}
\begin{itemize}
    \item Top Navigation: Contains a global search bar (``Search finasaan''), notification bell, and user profile management.
    \item Sidebar Menu:
    \begin{itemize}
        \item Home: The landing view.
        \item FinasaanGPT: Direct access to the AI financial assistant.
        \item Learn: Sub-menus for ``My Journeys'', ``Recommended'', ``Explore'', and ``Bookmarks''.
        \item Info: Expandable categories for ``Stocks'', ``Mutual Funds'', ``Insurance'', and ``Taxation''.
    \end{itemize}
    \item Layout: A clean, whitespace-rich main content area ready to display dynamic widgets, charts, or learning modules based on the user's selection.
\end{itemize}
