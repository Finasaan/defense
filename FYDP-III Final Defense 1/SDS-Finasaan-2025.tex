...
\textit{Image Description: A UML 2.5 behavioral state machine diagram showing user knowledge progression. States: Initial (filled circle) → Registration Complete → Beginner (entry: assessLevel) → Intermediate (trigger: conceptMastery > 60\%) → Advanced (trigger: conceptMastery > 85\%) → Final (bullseye circle). Self-transitions: Beginner state with trigger "chatInteraction" → updateMasteryScore; Intermediate state with trigger "moduleCompletion" → incrementProgress, awardBadge. Transitions include guard conditions in brackets [condition] and actions after slash /action. Alternative path: any state on error condition → Error State → resetProfile → Beginner. Include note boxes explaining mastery scoring. Use colors: light blue for beginner, green for intermediate, dark blue for advanced.}

\subsection{Portfolio Transaction State Machine}

\textbf{[PLACEHOLDER IMAGE: Transaction State Diagram]}

\textit{Image Description: A UML 2.5 state machine for investment transaction lifecycle. States: Initial → Initiated (entry: lockPrice) → Pending Validation (do: validateFunds) → Validated → Processing Payment → fork into parallel states: Confirming Exchange || Updating Portfolio || Generating Receipt → join → Completed → Final. Error paths: Pending Validation on validation failure → Failed Validation → Cancelled. Processing Payment on payment failure → Payment Failed → Refund Initiated → Refunded → Final. Include timeout transitions: Pending Validation after 10 min → Timeout → Cancelled. Transitions labeled with events and guards [conditions]. Color-code states: yellow initial, orange processing, green success, red errors.}
